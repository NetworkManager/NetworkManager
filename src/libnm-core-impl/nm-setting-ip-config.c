...
nm_ip_routing_rule_from_dbus(GVariant *variant, gboolean strict, GError **error)
{
    ...
    if (!variants[RR_DBUS_ATTR_FAMILY]) {
        g_set_error_literal(error,
                            NM_CONNECTION_ERROR,
                            NM_CONNECTION_ERROR_INVALID_PROPERTY,
                            _("missing \"%s\"", NM_IP_ROUTING_RULE_ATTR_FAMILY));
        return NULL;
    }
    ...
    if (!NM_IN_SET(addr_family, AF_INET, AF_INET6)) {
        g_set_error_literal(error,
                            NM_CONNECTION_ERROR,
                            NM_CONNECTION_ERROR_INVALID_PROPERTY,
                            _("invalid \"%s\"", NM_IP_ROUTING_RULE_ATTR_FAMILY));
        return NULL;
    }
        ...
        if (end < start) {
            if (strict) {
                g_set_error_literal(error,
                                    NM_CONNECTION_ERROR,
                                    NM_CONNECTION_ERROR_INVALID_PROPERTY,
                                    _("\"%s\" is greater than \"%s\"", NM_IP_ROUTING_RULE_ATTR_UID_RANGE_START, NM_IP_ROUTING_RULE_ATTR_UID_RANGE_END));
                return FALSE;
            }
        } else
