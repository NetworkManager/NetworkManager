...
after_interface_name:
    ...
    if (priv->ip_ping_addresses.arr && priv->ip_ping_addresses.arr->len > 0) {
        ...
        if (!priv->gateway_ping_timeout) {
            g_set_error_literal(error,
                                NM_CONNECTION_ERROR,
                                NM_CONNECTION_ERROR_INVALID_PROPERTY,
                                _("cannot be set if there is no ip-ping-timeout configured"));
            g_prefix_error(error,
                           "%s.%s: ",
                           NM_SETTING_CONNECTION_SETTING_NAME,
            ...
            if (addr_family == AF_INET) {
                ...
                if (s_ip4 && nm_setting_ip_config_get_may_fail(s_ip4)) {
                    g_set_error(error,
                                NM_CONNECTION_ERROR,
                                NM_CONNECTION_ERROR_INVALID_PROPERTY,
                                _("contains IPv4 address '%s', ipv4.may-fail cannot be 'true'"),
                                address);
                    g_prefix_error(error,
                                   "%s.%s: ",
                                   NM_SETTING_CONNECTION_SETTING_NAME,
                ...
                if (s_ip6 && nm_setting_ip_config_get_may_fail(s_ip6)) {
                    g_set_error(error,
                                NM_CONNECTION_ERROR,
                                NM_CONNECTION_ERROR_INVALID_PROPERTY,
                                _("contains IPv6 address '%s', ipv6.may-fail cannot be 'true'"),
                                address);
                    g_prefix_error(error,
                                   "%s.%s: ",
                                   NM_SETTING_CONNECTION_SETTING_NAME,
...
nm_setting_connection_class_init(NMSettingConnectionClass *klass)
{
     ...
     * in parallel. This property can not be used if connection.ip-ping-timeout is not
     ...
     * timeout is reached, or an IP gateway replies to a ping. This property also
     * specifies the timeout for the #NMSettingConnection:ip-ping-addresses.
     **/
    /* ---ifcfg-rh---
