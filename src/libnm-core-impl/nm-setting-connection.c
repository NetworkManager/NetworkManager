...
NM_GOBJECT_PROPERTIES_DEFINE(NMSettingConnection,
                             ...
                             PROP_IP_PING_ADDRESSES_TIMEOUT,
...
typedef struct {
    ...
    guint32     ip_ping_addresses_timeout;
    bool        autoconnect;
    bool        read_only;
...
/**
 * nm_setting_connection_get_ip_ping_addresses_timeout:
 * @setting: the #NMSettingConnection
 *
 * Returns: the value contained in the #NMSettingConnection:ip-ping-addresses-timeout
 * property.
 *
 * Since: 1.52
 **/
guint32
nm_setting_connection_get_ip_ping_addresses_timeout(NMSettingConnection *setting)
{
    g_return_val_if_fail(NM_IS_SETTING_CONNECTION(setting), 0);

    return NM_SETTING_CONNECTION_GET_PRIVATE(setting)->ip_ping_addresses_timeout;
}
...
after_interface_name:
    ...
    if (priv->ip_ping_addresses_timeout != 0
        ...
        g_prefix_error(error,
                       "%s.%s: ",
                       NM_SETTING_CONNECTION_SETTING_NAME,
                       NM_SETTING_CONNECTION_IP_PING_ADDRESSES_TIMEOUT);
        return FALSE;
    }
    ...
    if (priv->ip_ping_addresses.arr && priv->ip_ping_addresses.arr->len > 0) {
        ...
        if (priv->gateway_ping_timeout != 0 && priv->ip_ping_addresses_timeout != 0) {
            g_set_error(error,
                        NM_CONNECTION_ERROR,
                        NM_CONNECTION_ERROR_INVALID_PROPERTY,
...
nm_setting_connection_class_init(NMSettingConnectionClass *klass)
{
     ...
     * in parallel. This property can only be set if connection.ip-ping-addresses-timeout is
     * set. The ip-ping-addresses-timeout is used to delay the success of IP addressing until
    ...
    /**
     * NMSettingConnection:ip-ping-addresses-timeout:
     *
