                 ...
                 const char    *ifname)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        ret = nm_utils_dns_uri_parse(af_unspec ? AF_UNSPEC : addr_family, str, &dns);
        g_assert(ret);
        ...
        ret = nm_utils_dns_uri_parse(addr_family == AF_INET ? AF_INET6 : AF_INET, str, &dns);
        g_assert(!ret);
    }
}
...
test_dns_uri_parse(void)
{
    ...
    t_dns_1("dns+tls://8.8.8.8",                   INET,  DOT,  "8.8.8.8",            -1, NULL,      NULL);
    t_dns_1("dns+tls://1.2.3.4#name",              INET,  DOT,  "1.2.3.4",            -1, "name",    NULL);
    t_dns_1("dns+tls://1.2.3.4#a.b.c",             INET,  DOT,  "1.2.3.4",            -1, "a.b.c",   NULL);
    t_dns_1("dns+tls://1.2.3.4:53",                INET,  DOT,  "1.2.3.4",            53, NULL,      NULL);
    t_dns_1("dns+tls://1.2.3.4:53#foobar",         INET,  DOT,  "1.2.3.4",            53, "foobar",  NULL);
    t_dns_1("dns+tls://192.168.120.250:99",        INET,  DOT,  "192.168.120.250",    99, NULL,      NULL);
...
test_dns_uri_get_legacy(void)
{
    ...
    for (i = 0; i < G_N_ELEMENTS(values); i++) {
        ...
        result = nm_utils_dns_uri_get_plain(values[i].input_af,
                                            values[i].input,
                                            &addrstr,
                                            &to_free,
                                            &addr);
...
#define ADD_FUNC(name, func, secret_flags, comp_flags, remove_secret)           \
    ...
    g_test_add_func("/core/general/test_dns_uri_get_legacy", test_dns_uri_get_legacy);
