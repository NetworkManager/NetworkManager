...
merge_bridge_vlan_default_pvid(NMPlatformBridgeVlan *vlans, guint *num_vlans, guint default_pvid)
{
    ...
    if (has_pvid && vlan) {
        /* The port already contains the VLAN ID and has a PVID set: nothing to do */
    } else if (vlan) {
        /* The port already contains the VLAN ID but without the PVID flag: set the PVID flag */

        if (vlan->vid_start == vlan->vid_end) {
            /* If it's a single VLAN, just set PVID flag. */
            vlan->pvid = TRUE;
        } else {
            ...
            *vlan = (NMPlatformBridgeVlan){
            ...
            if (default_pvid != old_range.vid_start) {
                (*num_vlans)++;
                vlans[*num_vlans - 1] = (NMPlatformBridgeVlan){
                    .vid_start = old_range.vid_start,
                    .vid_end   = default_pvid - 1,
                    .untagged  = old_range.untagged,
                    .pvid      = FALSE,
                };
            }
            if (default_pvid != old_range.vid_end) {
                (*num_vlans)++;
                vlans[*num_vlans - 1] = (NMPlatformBridgeVlan){
                    .vid_start = default_pvid + 1,
