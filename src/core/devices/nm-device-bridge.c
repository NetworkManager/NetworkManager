...
static NMPlatformBridgeVlan *
merge_bridge_vlan_pvid(NMPlatformBridgeVlan *vlans, guint *num_vlans, guint pvid)
{
    ...
    vlan = NULL;
    for (i = 0; i < *num_vlans; i++) {
        if (pvid >= vlans[i].vid_start && pvid <= vlans[i].vid_end) {
            vlan = &vlans[i];
            break;
        }
    }
    ...
    if (vlan) {
        if (vlan->vid_start == vlan->vid_end) {
            /* If it's a single VLAN, just set PVID flag unless there
             * already another explicit PVID. */
            vlan->pvid = !has_pvid;
        } else {
            NMPlatformBridgeVlan old_range = *vlan;

            if (has_pvid) {
                /* The VLAN already exists and doesn't need the PVID
                 * flag. Nothing to do. */
            } else {
                /* Replace the range with the PVID VLAN and append
                 * the remaining VLAN ranges at the end. */
                *vlan = (NMPlatformBridgeVlan){
                    .vid_start = pvid,
                    .vid_end   = pvid,
                    .untagged  = TRUE,
                    .pvid      = TRUE,
                };
                vlans = g_realloc_n(vlans, *num_vlans + 2, sizeof(NMPlatformBridgeVlan));

                if (pvid != old_range.vid_start) {
                    (*num_vlans)++;
                    vlans[*num_vlans - 1] = (NMPlatformBridgeVlan){
                        .vid_start = old_range.vid_start,
                        .vid_end   = pvid - 1,
                        .untagged  = old_range.untagged,
                        .pvid      = FALSE,
                    };
                }
                if (pvid != old_range.vid_end) {
                    (*num_vlans)++;
                    vlans[*num_vlans - 1] = (NMPlatformBridgeVlan){
                        .vid_start = pvid + 1,
                        .vid_end   = old_range.vid_end,
                        .untagged  = old_range.untagged,
                        .pvid      = FALSE,
                    };
                }
...
nm_device_reapply_bridge_port_vlans(NMDevice *device)
{
    ...
    if (nm_setting_bridge_get_vlan_filtering(s_bridge)) {
        ...
        setting_vlans = merge_bridge_vlan_pvid(setting_vlans,
                                               &num_setting_vlans,
    ...
    if (!nm_platform_link_get_bridge_vlans(plat, ifindex, &plat_vlans, &num_plat_vlans)) {
        _LOGT(LOGD_DEVICE, "reapply-bridge-port-vlans: can't get current VLANs from platform");
        do_reapply = TRUE;
    } else if (!nmp_utils_bridge_vlans_equal(setting_vlans,
                                             num_setting_vlans,
                                             plat_vlans,
                                             num_plat_vlans)) {
        _LOGT(LOGD_DEVICE, "reapply-bridge-port-vlans: VLANs in platform need reapply");
        do_reapply = TRUE;
    } else {
        _LOGT(LOGD_DEVICE, "reapply-bridge-port-vlans: VLANs in platform didn't change");
        do_reapply = FALSE;
    }
