...
schedule_peer_list_dump(NMDeviceIwdP2P *self)
{
    ...
    if (!priv->peer_dump_source && _LOGD_ENABLED(LOGD_WIFI_SCAN)) {
        priv->peer_dump_source =
            nm_g_timeout_source_new_seconds(1, G_PRIORITY_DEFAULT, peer_list_dump, self, NULL);
        g_source_attach(priv->peer_dump_source, NULL);
    }
}
...
iwd_request_discovery(NMDeviceIwdP2P *self, unsigned timeout)
{
    ...
    priv->find_peer_timeout_source = nm_g_timeout_source_new_seconds(timeout,
                                                                     G_PRIORITY_DEFAULT,
                                                                     iwd_discovery_timeout_cb,
                                                                     self,
                                                                     NULL);
    g_source_attach(priv->find_peer_timeout_source, NULL);
...
iwd_wsc_connect_cb(GObject *source, GAsyncResult *res, gpointer user_data)
{
    ...
    /* We need to go through stage 3 but when PushButton/StartPin() return and
     * we get here, we know that the IP is already configured and the connection
     * is ready.  The peer interface's "Connected" Property is also about to
     * flip to True at the same time.
     */
