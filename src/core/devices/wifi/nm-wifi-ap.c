...
static gboolean
nm_wifi_ap_set_wifi_standard(NMWifiAP *ap, NMWifiStandard wifi_standard)
{
    NMWifiAPPrivate *priv = NM_WIFI_AP_GET_PRIVATE(ap);

    if (priv->wifi_standard != wifi_standard) {
        priv->wifi_standard = wifi_standard;
        _notify(ap, PROP_WIFI_STANDARD);
        return TRUE;
    }
    return FALSE;
}
...
nm_wifi_ap_update_from_properties(NMWifiAP *ap, const NMSupplicantBssInfo *bss_info)
{
    ...
    changed |= nm_wifi_ap_set_wifi_standard(ap, bss_info->wifi_standard);
    ...
    case PROP_LAST_SEEN:
        g_value_set_int(
            ...
            priv->last_seen_msec != G_MININT64
                ...
                : -1);
        break;
    case PROP_WIFI_STANDARD:
        g_value_set_uint(value, priv->wifi_standard);
        break;
    default:
...
nm_wifi_ap_init(NMWifiAP *self)
{
    ...
    priv->last_seen_msec = G_MININT64;
    priv->wifi_standard  = NM_WIFI_STANDARD_LEGACY;
}
...
static const NMDBusInterfaceInfoExtended interface_info_access_point = {
    .parent = NM_DEFINE_GDBUS_INTERFACE_INFO_INIT(
        ...
        .properties = NM_DEFINE_GDBUS_PROPERTY_INFOS(
            ...
            NM_DEFINE_DBUS_PROPERTY_INFO_EXTENDED_READABLE("WifiStandard",
                                                           "u",
                                                           NM_WIFI_AP_WIFI_STANDARD), ), ),
};
...
#define ALL_SEC_FLAGS                                                                            \
    ...
    obj_properties[PROP_WIFI_STANDARD] =
        g_param_spec_uint(NM_WIFI_AP_WIFI_STANDARD,
                          "",
                          "",
                          NM_WIFI_STANDARD_LEGACY,
                          G_MAXUINT,
                          NM_WIFI_STANDARD_LEGACY,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
