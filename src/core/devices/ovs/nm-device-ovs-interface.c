...
{
    NMDeviceOvsInterfaceCheck   *cb_data = user_data;
    NMDevice                    *device  = cb_data->device;
    NMDeviceOvsInterface        *self    = NM_DEVICE_OVS_INTERFACE(device);
    NMDeviceOvsInterfacePrivate *priv    = NM_DEVICE_OVS_INTERFACE_GET_PRIVATE(self);

    priv->waiting_for_interface = FALSE;
    nm_device_set_ip_ifindex(device, cb_data->ifindex);
                    ...
                    NMDevice       *device)
{
    ...
    NMDeviceOvsInterfaceCheck       *cb_data     = NULL;

    if (change_type == NM_PLATFORM_SIGNAL_ADDED) {
        if (pllink->type == NM_LINK_TYPE_TUN
            && nm_streq0(pllink->name, nm_device_get_iface(device))) {
            g_signal_handlers_disconnect_by_func(platform, _netdev_tun_link_cb, device);
            cb_data          = g_slice_new0(NMDeviceOvsInterfaceCheck);
            cb_data->device  = device;
            cb_data->ifindex = ifindex;

            nm_g_idle_add(_set_ip_ifindex_tun, cb_data);
        }
    }
}
