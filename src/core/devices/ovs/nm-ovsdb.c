...
static json_t *
create_port_row_object(NMConnection *connection, gboolean is_reapply)
{
    ...
    json_object_set_new(row, "tag", json_integer(u));
...
_insert_port(json_t *params, NMConnection *port, json_t *new_interfaces)
{
    ...
    row = create_port_row_object(port, FALSE);
    ...
    default:
    {
        ...
        case OVSDB_SET_REAPPLY:
        {
            ...
            case NM_DEVICE_TYPE_OVS_PORT:
                row = create_port_row_object(connection, TRUE);
                break;
            default:
                row = NULL;
                break;
