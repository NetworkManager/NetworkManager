...
static void
_configure_firewall(NMDeviceWireGuard *self, int addr_family, gboolean up)
{
    NMDeviceWireGuardPrivate *priv = NM_DEVICE_WIREGUARD_GET_PRIVATE(self);
    NMConnection             *connection;
...
deactivate(NMDevice *device)
{
    ...
    _configure_firewall(self, AF_INET, FALSE);
    _configure_firewall(self, AF_INET6, FALSE);
}
...
act_stage3_ip_config(NMDevice *device, int addr_family)
{
    ...
    _configure_firewall(NM_DEVICE_WIREGUARD(device), addr_family, TRUE);
