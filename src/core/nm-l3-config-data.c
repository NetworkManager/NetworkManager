...
struct _NML3ConfigData {
    ...
    bool add_dns_routes_4 : 1;
    bool add_dns_routes_6 : 1;
};
...
#define _L(...)                                      \
    ...
    if (self->add_dns_routes_4)
        _L("add-dns-routes4: yes");
    if (self->add_dns_routes_6)
        _L("add-dns-routes6: yes");
...
gboolean
nm_l3_config_data_get_add_dns_routes(const NML3ConfigData *self, int addr_family)
{
    const int IS_IPv4 = NM_IS_IPv4(addr_family);

    nm_assert(_NM_IS_L3_CONFIG_DATA(self, TRUE));
    if (IS_IPv4) {
        return self->add_dns_routes_4;
    } else {
        return self->add_dns_routes_6;
    }
}

void
nm_l3_config_data_set_add_dns_routes(NML3ConfigData *self, int addr_family, gboolean value)
{
    const int IS_IPv4 = NM_IS_IPv4(addr_family);

    nm_assert(_NM_IS_L3_CONFIG_DATA(self, FALSE));
    if (IS_IPv4) {
        self->add_dns_routes_4 = value;
    } else {
        self->add_dns_routes_6 = value;
    }
}
                           ...
                           NML3ConfigCmpFlags    flags)
{
    ...
    if (NM_FLAGS_HAS(flags, NM_L3_CONFIG_CMP_FLAGS_OTHER)) {
        ...
        NM_CMP_DIRECT_UNSAFE(a->add_dns_routes_4, b->add_dns_routes_4);
        NM_CMP_DIRECT_UNSAFE(a->add_dns_routes_6, b->add_dns_routes_6);
...
#undef _ensure_r
    ...
    if (src->add_dns_routes_4)
        self->add_dns_routes_4 = TRUE;
    if (src->add_dns_routes_6)
        self->add_dns_routes_6 = TRUE;
}
