...
#define DNS_ROUTES_FWMARK_TABLE  4500
#define DNS_ROUTES_RULE_PRIORITY 22000

static void
_l3cfg_add_dns_routes(NML3Cfg *self, NML3ConfigData *l3cd)
{
    for (int IS_IPv4 = 1; IS_IPv4 >= 0; IS_IPv4--) {
        ...
        if (!nm_l3_config_data_get_add_dns_routes(l3cd, addr_family)) {
            /* Even if we are not adding new DNS routes, we did previously.
             * Therefore, we need to set the routing table sync mode to FULL,
             * so that we clear the DNS routes added previously. */
        ...
        for (i = 0; i < len; i++) {
             ...
             * before. For this reason, use policy routing with special fwmark
             * that skips the table containing DNS routes. */
            r = nm_platform_ip_route_get(self->priv.platform,
                                         addr_family,
                                         &addr,
                                         DNS_ROUTES_FWMARK_TABLE,
                                         self->priv.ifindex,
                                         &obj);
            if (r < 0) {
            ...
            if (IS_IPv4) {
                route_new.r4 = (NMPlatformIP4Route){
                    ...
                    .table_any     = FALSE,
                    .metric_any    = TRUE,
                    .table_coerced = nm_platform_route_table_coerce(DNS_ROUTES_FWMARK_TABLE),
        ...
        if (route_added) {
            ...
            rule = ((NMPlatformRoutingRule){
                ...
                .priority    = DNS_ROUTES_RULE_PRIORITY,
                .table       = DNS_ROUTES_FWMARK_TABLE,
                .fwmark      = DNS_ROUTES_FWMARK_TABLE,
                .fwmask      = 0xffffffff,
                              ...
                              gboolean              *out_changed_combined_l3cd)
{
    ...
    if (l3_config_datas_len > 0) {
        ...
        _l3cfg_add_dns_routes(self, l3cd);
