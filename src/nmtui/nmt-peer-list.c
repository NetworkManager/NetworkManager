...
/**
 * SECTION:nmt-peer-list:
 * @short_description: An editable list of a connection's peers
 *
 * #NmtPeerList implements an #NmtNewtGrid for the
...
G_DEFINE_TYPE(NmtPeerList, nmt_peer_list, NMT_TYPE_NEWT_GRID)

#define NMT_PEER_LIST_GET_PRIVATE(o) \
    (G_TYPE_INSTANCE_GET_PRIVATE((o), NMT_TYPE_PEER_LIST, NmtPeerListPrivate))
 ...
 *
 * Creates a new #NmtPeerList.
 *
 * Returns: a new #NmtPeerList.
 */
NmtNewtWidget *
nmt_peer_list_new(NMSettingWireGuard *setting)
{
    return g_object_new(NMT_TYPE_PEER_LIST, "setting", setting, NULL);
}
...
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(list);
    NMConnection *      connection;

    connection = nmt_newt_listbox_get_active_key(priv->listbox);
    g_return_if_fail(connection != NULL);

    g_signal_emit(list, signals[EDIT_PEER], 0, connection);
}
...
static void
nmt_peer_list_init(NmtPeerList *list)
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(list);
...
static void nmt_peer_list_rebuild(NmtPeerList *list);

static void
rebuild_on_peer_changed(NMRemoteConnection *connection, gpointer list)
{
    nmt_peer_list_rebuild(list);
}

static void
free_peers(NmtPeerList *list)
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(list);
    ...
    for (iter = priv->peers; iter; iter = iter->next) {
        ...
        g_signal_handlers_disconnect_by_func(peer, G_CALLBACK(rebuild_on_peer_changed), list);
        g_object_unref(peer);
    }
...
static void
nmt_peer_list_finalize(GObject *object)
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(object);

    free_peers(NMT_PEER_LIST(object));
    g_object_unref(priv->setting);
    g_object_unref(priv->peers);

    G_OBJECT_CLASS(nmt_peer_list_parent_class)->finalize(object);
}

static void
nmt_peer_list_add_peer(NmtPeerList *list)
{}

static void
nmt_peer_list_edit_peer(NmtPeerList *list, NMConnection *connection)
{}

static void
nmt_peer_list_remove_peer(NmtPeerList *list, NMConnection *connection)
{}

static void
nmt_peer_list_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(object);
...
static void
nmt_peer_list_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(object);
    GPtrArray *         peers;
    GSList *            iter;
...
static void
nmt_peer_list_rebuild(NmtPeerList *list)
{
    NmtPeerListPrivate *priv = NMT_PEER_LIST_GET_PRIVATE(list);
    GSList *            iter;
    ...
    for (i = 0; i < num; i++) {
        ...
        //               list);
        priv->peers = g_slist_prepend(priv->peers, g_object_ref(peer));
    }
    ...
    for (iter = priv->peers, row = 0; iter; iter = iter->next, row++) {
        peer = iter->data;
        nmt_newt_listbox_append(priv->listbox, "peer", peer);
...
static void
nmt_peer_list_constructed(GObject *object)
{
    NmtPeerList *list = NMT_PEER_LIST(object);
    ...
    G_OBJECT_CLASS(nmt_peer_list_parent_class)->constructed(object);
}

static void
nmt_peer_list_class_init(NmtPeerListClass *list_class)
{
    GObjectClass *object_class = G_OBJECT_CLASS(list_class);

    g_type_class_add_private(list_class, sizeof(NmtPeerListPrivate));

    /* virtual methods */
    object_class->constructed  = nmt_peer_list_constructed;
    object_class->set_property = nmt_peer_list_set_property;
    object_class->get_property = nmt_peer_list_get_property;
    object_class->finalize     = nmt_peer_list_finalize;

    list_class->add_peer    = nmt_peer_list_add_peer;
    list_class->edit_peer   = nmt_peer_list_edit_peer;
    list_class->remove_peer = nmt_peer_list_remove_peer;

    /* signals */

    /**
     * NmtPeerList::add-connection:
     * @list: the #NmtPeerList
     *
     * Emitted when the user clicks the list's "Add" button.
     */
    signals[ADD_PEER] = g_signal_new("add-connection",
                                     G_OBJECT_CLASS_TYPE(object_class),
                                     G_SIGNAL_RUN_FIRST,
                                     G_STRUCT_OFFSET(NmtPeerListClass, add_peer),
                                     NULL,
                                     NULL,
                                     NULL,
                                     G_TYPE_NONE,
     ...
     */
    signals[EDIT_PEER] = g_signal_new("edit-connection",
                                      G_OBJECT_CLASS_TYPE(object_class),
                                      G_SIGNAL_RUN_FIRST,
                                      G_STRUCT_OFFSET(NmtPeerListClass, edit_peer),
                                      NULL,
                                      NULL,
                                      NULL,
                                      G_TYPE_NONE,
    ...
    /**
     * NmtPeerList::remove-connection:
     * @list: the #NmtPeerList
     * @connection: the connection to remove
     *
     * Emitted when the user clicks the list's "Delete" button.
     */
    signals[REMOVE_PEER] = g_signal_new("remove-connection",
                                        G_OBJECT_CLASS_TYPE(object_class),
                                        G_SIGNAL_RUN_FIRST,
                                        G_STRUCT_OFFSET(NmtPeerListClass, remove_peer),
                                        NULL,
                                        NULL,
                                        NULL,
                                        G_TYPE_NONE,
