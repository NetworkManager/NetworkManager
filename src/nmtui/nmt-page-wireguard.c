...
nmt_page_wireguard_constructed(GObject *object)
{
    ...
    s_wireguard = NM_SETTING_WIREGUARD(
        nm_connection_get_setting_by_name(conn, NM_SETTING_WIREGUARD_SETTING_NAME));
    if (!s_wireguard) {
    ...
    priv->private_key = NMT_NEWT_ENTRY(widget);
    ...
    widget = nmt_peer_list_new(s_wireguard);
    g_signal_connect(widget, "notify::connections", G_CALLBACK(NULL), wireguard);
    nmt_editor_grid_append(grid, NULL, widget, NULL);
    priv->peers = NMT_PEER_LIST(widget);
