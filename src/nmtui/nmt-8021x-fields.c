...
static NmtNewtPopupEntry pe_tristate[] =
   {{N_("Default"), ""},
    {N_("Disable"), "disable"},
    {N_("Enable"),  "enable"},
    {NULL, NULL}};

static NmtNewtPopupEntry pe_disable[] =
   {{N_("Default"), ""},
    {N_("Disable"), "disable"},
    {NULL, NULL}};

typedef struct _Auth_Flags_Tls Auth_Flags_Tls;
struct _Auth_Flags_Tls {
    const char *label;
...
static gboolean
phase1_auth_flag_get_bit(GBinding* binding, const GValue *from_value, GValue *to_value, gpointer user_data)
{
    Auth_Flags_Tls *af = (Auth_Flags_Tls *)user_data;
    unsigned src = g_value_get_uint(from_value);
    if (src & af->disable) g_value_set_string(to_value, "disable");
    else if (src & af->enable) g_value_set_string(to_value, "enable");
    else g_value_set_string(to_value, NULL);
    return TRUE;
}

static gboolean
phase1_auth_flag_set_bit(GBinding* binding, const GValue *from_value, GValue *to_value, gpointer user_data)
{
    Auth_Flags_Tls *af = (Auth_Flags_Tls *)user_data;
    GObject *from_object = g_binding_get_source(binding);
    if (from_object == NULL) return FALSE;
    GValue orig_value;
    g_value_init(&orig_value,G_TYPE_UINT);
    g_object_get_property(from_object, g_binding_get_source_property(binding), &orig_value);
    unsigned bits = g_value_get_uint(&orig_value);
    g_value_unset(&orig_value);
    const char *val = g_value_get_string(from_value);
    bits = bits & ~(unsigned)(af->enable | af->disable);
    if (val) switch(val[0]) {
        case 'd': bits |= af->disable; break;
        case 'e': bits |= af->enable; break;
    }
...
eap_populate_advanced_tls_settings(EapMethod *method, NmtNewtWidget *subgrid)
{
    ...
    const char *oc;
    static Auth_Flags_Tls auth_flag_switches[] = {
...
#undef ENDIS_TLS
    ...
    has_advanced_settings = (oc != NULL) && !!strlen(oc) || !!nm_setting_802_1x_get_phase1_auth_flags(method->setting);
    ...
    g_signal_connect(widget, "notify::active", G_CALLBACK(checkbox_advanced_tls_settings_changed), method);
    ...
    for (Auth_Flags_Tls* sw = auth_flag_switches; sw->states; sw++) {
