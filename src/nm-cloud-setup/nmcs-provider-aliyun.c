...
typedef enum {
    GET_CONFIG_FETCH_DONE_TYPE_SUBNET_IPV4_CIDR_BLOCK,
    GET_CONFIG_FETCH_DONE_TYPE_LOCAL_IPV4S,
    GET_CONFIG_FETCH_DONE_TYPE_SUBNET_NETMASK
                          ...
                          GetConfigFetchDoneType fetch_type)
{
    ...
    switch (fetch_type) {
    case GET_CONFIG_FETCH_DONE_TYPE_LOCAL_IPV4S:
    ...
    case GET_CONFIG_FETCH_DONE_TYPE_SUBNET_IPV4_CIDR_BLOCK:
    ...
    case GET_CONFIG_FETCH_DONE_TYPE_SUBNET_NETMASK:
...
static void
_get_config_fetch_done_cb_subnet_netmask(GObject *source, GAsyncResult *result, gpointer user_data)
{
    _get_config_fetch_done_cb(NM_HTTP_CLIENT(source),
                              result,
                              user_data,
...
_get_config_metadata_ready_cb(GObject *source, GAsyncResult *result, gpointer user_data)
{
    ...
    while (g_hash_table_iter_next(&h_iter, (gpointer *) &v_hwaddr, (gpointer *) &v_mac_data)) {
        ...
        nm_http_client_poll_get(
            ...
            NULL,
            NULL,
            _get_config_fetch_done_cb_subnet_netmask,
            nm_utils_user_data_pack(get_config_data, hwaddr));
    }
