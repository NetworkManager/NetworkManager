...
test_vlan_invalid(void)
{
...

    {
        ...
        const char *const             *ARGV0       = NM_MAKE_STRV("vlan=myvlan99999:eth0");
        gs_unref_hashtable GHashTable *connections = NULL;

        NMTST_EXPECT_NM_WARN("cmdline-reader: invalid VLAN id '99999'");
        connections = _parse_cons(ARGV0);
        g_assert_cmpint(g_hash_table_size(connections), ==, 0);
        g_test_assert_expected_messages();
    }
...
main(int argc, char **argv)
{
    ...
    g_test_add_func("/initrd/cmdline/vlan/invalid-id", test_vlan_invalid);
