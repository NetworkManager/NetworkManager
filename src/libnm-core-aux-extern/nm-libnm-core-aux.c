    ...
    for (i = 0; tokens[i]; i++) {
        if (i == 0) {
            if (!nm_wireguard_peer_set_public_key(peer, tokens[i], FALSE)) {
                g_set_error(error, 1, 0, "invalid public key '%s'", tokens[i]);
                return NULL;
            }
            continue;
        }
        ...
        if (!value || value[1] == '\0') {
            g_set_error(error, 1, 0, "attribute without value '%s'", tokens[i]);
            return NULL;
        }
        ...
        if (nm_streq(tokens[i], "allowed-ips")) {
            ...
            for (j = 0; ips[j]; j++) {
                if (!nm_wireguard_peer_append_allowed_ip(peer, ips[j], FALSE)) {
                    g_set_error(error, 1, 0, "invalid allowed-ip '%s'", ips[j]);
                    return NULL;
                }
            }
        } else if (nm_streq(tokens[i], "endpoint")) {
            if (!nm_wireguard_peer_set_endpoint(peer, value, FALSE)) {
                g_set_error(error, 1, 0, "invalid endpoint '%s'", value);
                return NULL;
            }
            ...
            if (keepalive == -1) {
                g_set_error(error, 1, 0, "invalid persistent-keepalive value '%s'", value);
                return NULL;
            }
            ...
            if (!nm_wireguard_peer_set_preshared_key(peer, value, FALSE)) {
                g_set_error(error, 1, 0, "invalid preshared-key '%s'", value);
                return NULL;
            }
            has_psk = TRUE;
            ...
            if (!nm_utils_enum_from_str(NM_TYPE_SETTING_SECRET_FLAGS, value, &flags, NULL)) {
                g_set_error(error, 1, 0, "invalid preshared-key-flags '%s'", value);
                return NULL;
            }
            nm_wireguard_peer_set_preshared_key_flags(peer, (NMSettingSecretFlags) flags);
            has_psk_flags = TRUE;
        } else {
            g_set_error(error, 1, 0, "invalid attribute '%s'", tokens[i]);
            return NULL;
        }
    }
