...
nm_ip6_addr_rfc6724_label(const struct in6_addr *addr)
{
    ...
    /* ::ffff:0:0/96 (IPv4-mapped) -> label 4 */
    if (IN6_IS_ADDR_V4MAPPED(addr))
        return 4;

    /* ::/96 (IPv4-compatible, deprecated) -> label 3 */
    ...
    /* 3ffe::/16 -> label 12 (deprecated 6bone) */
    if ((addr->s6_addr32[0] & htonl(0xFFFF0000u)) == htonl(0x3FFE0000u))
        return 12;

    /* fec0::/10 -> label 11 (deprecated site-local) */
