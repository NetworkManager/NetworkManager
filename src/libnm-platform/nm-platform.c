...
static const char *
_nmp_link_port_kind_to_string(const NMPortKind port_kind, char buf[30])
{
    switch (port_kind) {
...
static const char *
_nmp_link_bond_port_to_string(const NMPlatformLinkBondPort *bond_port, char buf[100])
{
    nm_assert(bond_port);
                        ...
                        NMPlatformLinkChangeFlags flags)
{
    ...
    return klass->link_change(self, ifindex, props, bond_port, flags);
}
...
nm_platform_link_to_string(const NMPlatformLink *link, char *buf, gsize len)
{
    ...
    char        str_port_kind[30];
    char        str_bond_port[100];
    ...
    _nmp_link_bond_port_to_string(&link->bond_port_opts, str_bond_port);

    str_link_type = nm_link_type_to_string(link->type);

    if (link->port_kind)
        _nmp_link_port_kind_to_string(link->port_kind, str_port_kind);
...
nm_platform_link_hash_update(const NMPlatformLink *obj, NMHashState *h)
{
    ...
    nm_platform_link_bond_port_hash_update(&obj->bond_port_opts, h);
}
...
nm_platform_link_cmp(const NMPlatformLink *a, const NMPlatformLink *b)
{
    ...
    NM_CMP_RETURN(nm_platform_link_bond_port_cmp(&a->bond_port_opts, &b->bond_port_opts));
