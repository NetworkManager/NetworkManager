...
static gboolean
_dcb_parse_app_table(GArray *out, struct nlattr *app_attr)
{
    static const struct nla_policy policy[] = {[DCB_ATTR_IEEE_APP_UNSPEC] = {.type = NLA_UNSPEC},
                                               [DCB_ATTR_IEEE_APP]        = {.type = NLA_UNSPEC},
                                               [DCB_ATTR_DCB_APP]         = {.type = NLA_UNSPEC}};
    struct nlattr                 *tb[G_N_ELEMENTS(policy)];
    struct nlattr                 *app;
    void                          *elem;

    if (nla_parse_nested_arr(tb, app_attr, policy) < 0)
        return FALSE;

    if (tb[DCB_ATTR_IEEE_APP_UNSPEC])
        app = tb[DCB_ATTR_IEEE_APP_UNSPEC];
    else if (tb[DCB_ATTR_IEEE_APP])
        app = tb[DCB_ATTR_IEEE_APP];
    else if (tb[DCB_ATTR_DCB_APP])
        app = tb[DCB_ATTR_DCB_APP];
    else
        return false;

    elem = nm_g_array_append_new(out, struct dcb_app);
    nla_memcpy(&elem, app, sizeof(struct dcb_app));

    return true;
}

static int
_dcb_app_table_get_cb(const struct nl_msg *msg, void *arg)
{
    static const struct nla_policy policy[] = {[DCB_ATTR_IEEE_APP_TABLE] = {.type = NLA_NESTED}};
    struct nlattr                 *tb[G_N_ELEMENTS(policy)];
    ...
    if (tb[DCB_ATTR_IEEE_APP_TABLE]) {
        ...
        apps = g_array_new(TRUE, FALSE, sizeof(struct dcb_app));

        nla_for_each_nested (attr, tb[DCB_ATTR_IEEE_APP_TABLE], rem) {
            if (!_dcb_parse_app_table(apps, attr)) {
                g_free(apps);
                return NL_SKIP;
            }
        }
...
dcb_set_dcbx(NMPlatform *platform, const char *const ifname, guint8 mode)
{
    ...
    r = nl_recvmsgs(sk,
                    &((const struct nl_cb) {
                        .valid_cb  = _dcb_app_table_get_cb,
                        .valid_arg = NULL,
                    }));
