...
do_device_wifi_connect_network (NmCli *nmc, int argc, char **argv)
{
	...
	for (i = 0; i < avail_cons->len; i++) {
		NMRemoteConnection *avail_con = g_ptr_array_index (avail_cons, i);
		NMSettingWireless *s_wireless = nm_connection_get_setting_wireless (NM_CONNECTION (avail_con));
		const GByteArray *bssid_arr = bssid2_arr;
		...
		if (!bssid1_arr) {
			...
			if (!utf8 || strcmp (utf8, param_user))
				continue;

			g_free (utf8);
		} else
			bssid_arr = bssid1_arr;

		if (bssid_arr) {
			const gchar *con_bssid_str = nm_setting_wireless_get_bssid (s_wireless);
			GByteArray *con_bssid;

			if (!con_bssid_str)
				continue;

			con_bssid = nm_utils_hwaddr_atoba (con_bssid_str, ETH_ALEN);
			if (!con_bssid)
				continue;

			if (memcmp (bssid_arr->data, con_bssid->data, ETH_ALEN)) {
				g_byte_array_free (con_bssid, TRUE);
				continue;
			}
