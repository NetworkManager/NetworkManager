...
#define peers_psk_get_secret_parse_a(secret_public_key, public_key_free) \
	({ \
		...
		\
		if (NM_STR_HAS_PREFIX (_secret_public_key, "peers.")) { \
			_secret_public_key += NM_STRLEN ("peers."); \
			if (g_str_has_suffix (_secret_public_key, ".preshared-key")) { \
				_public_key = nm_strndup_a (300, _secret_public_key, strlen (_secret_public_key) - NM_STRLEN (".preshared-key"), _public_key_free); \
			} \
		} \
		\
		_public_key; \
...
get_secret_flags (NMSetting *setting,
                  ...
                  GError **error)
{
	if (NM_STR_HAS_PREFIX (secret_name, "peers.")) {
		NMSettingWireGuardPrivate *priv = NM_SETTING_WIREGUARD_GET_PRIVATE (setting);
